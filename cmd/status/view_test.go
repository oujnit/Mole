...
func TestStatusDiagnosisLineDoesNotNameStaleCPUProcess(t *testing.T) {
	...
	if got := statusDiagnosisLine(m); got != "处理器负载过高" {
...
func TestStatusDiagnosisLineDoesNotNameUnknownMemoryProcess(t *testing.T) {
	...
	if got := statusDiagnosisLine(m); got != "内存压力过高" {
...
func TestBuildCardsMarksStaleProcessSampleAtNarrowAndWideWidths(t *testing.T) {
	...
	for _, width := range []int{5, 19, 20, 56} {
		...
		if !strings.Contains(rendered, "过期") {
...
func TestBuildCardsMarksUnknownProcessFreshness(t *testing.T) {
	...
	if !strings.Contains(rendered, "未知") || !strings.Contains(rendered, "Xcode") {
...
func TestBuildCardsDistinguishesEmptyProcessSampleStates(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "fresh empty", snapshot: MetricsSnapshot{ProcessStale: &fresh}, want: "无进程活动"},
		{name: "fresh alert only", snapshot: MetricsSnapshot{ProcessStale: &fresh, ProcessAlerts: []ProcessAlert{{Status: "active"}}}, want: "无进程数据 · 上方有活动告警"},
		{name: "stale empty", snapshot: MetricsSnapshot{ProcessStale: &stale}, want: "已过期 · 未保留进程活动"},
		{name: "stale measured zero", snapshot: MetricsSnapshot{ProcessStale: &stale, ZombieCount: &zero}, want: "已过期 · 未保留进程活动"},
		{name: "unknown retained alert", snapshot: MetricsSnapshot{ProcessAlerts: []ProcessAlert{{Status: "active"}}}, want: "未知 · 进程样本不可用"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if strings.Contains(rendered, "正在采集") {
...
func TestRenderMemoryCardUsesCollectedAvailableMemory(t *testing.T) {
	...
	if !strings.Contains(plain, "可用   9.0 GB") {
